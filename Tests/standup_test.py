...
def test_standup_start_unauthorised_user_access(_setup):
    ...
    with pytest.raises(AccessError):
        standup_start(token=user_2_token, channel_id=channel_id, length=10)
...
def test_standup_start_time_finish_greater_than_time_current(_setup):
    ...
    time_finish = standup_start(token=user_1_token, channel_id=channel_id, length=10)

    #Testing if time_finish is greater than the time_current
    assert (time_finish > time_current) is True


def test_standup_start_correct_time_difference_5_seconds(_setup):
    '''Testing standup_start and checking for correct time difference of 5 seconds'''
    time_current = datetime.datetime.now()

    user_1_token = _setup['user_1_token']
    channel_id = _setup['channel_created_by_user_1']

    time_finish = standup_start(token=user_1_token, channel_id=channel_id, length=5)
...
def test_standup_start_correct_time_difference_15_seconds(_setup):
    '''Testing standup_start and checking for correct time difference of 15 seconds'''
    time_current = datetime.datetime.now()

    user_1_token = _setup['user_1_token']
    channel_id = _setup['channel_created_by_user_1']

    time_finish = standup_start(token=user_1_token, channel_id=channel_id, length=15)

    time_difference = time_finish - time_current

    #Testing if time_finish has a 15 minute differnce in time from time_current
    assert (time_difference.seconds) == 15
...
def test_standup_active_valid(_setup):
    ...
    standup_start(token=user_token, channel_id=channel_id_1, length=10)
...
def test_standup_send_message_too_long(_setup):
    ...
    user_token = _setup['user_1_token']
    channel_id = _setup['channel_created_by_user_1']
    standup_start(token=user_token, channel_id=channel_id, length=10)
